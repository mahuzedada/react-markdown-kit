...
`remarkPlugins={[remarkGfm]}`, and through plain CommonMark. The first two must produce
byte-identical markup and the third must not produce the GFM markup, so a case that would
pass without GFM cannot sit in the corpus.
...
because parsing is roughly 80% of the work.
