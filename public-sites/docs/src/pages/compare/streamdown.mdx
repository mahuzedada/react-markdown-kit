...
`remarkPlugins={[remarkGfm]}`, and through plain CommonMark, where the first two must be
byte-identical and the third must not produce the GFM markup. CommonMark itself is
...
Mermaid.js (<Link to="/react-mermaid">Mermaid in React Markdown</Link>). It ships no math
...
The design difference is what an incomplete construct looks like. Streamdown suppresses
it. The kit shows the literal characters and guarantees the closed prefix above does not
move. If you want suppression, do it in your component by trimming the tail before you
hand the string over; the kit does not do it for you.
...
roughly 80% of the work. A finished chat message is exactly the case for it.
...
- **Mermaid.** The kit's plugin renders flowcharts only and loads no Mermaid.js
