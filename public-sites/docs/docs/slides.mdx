---
title: Markdown Presentations in React
sidebar_label: Slides
description: "Markdown presentations in React: slides split on ---, speaker notes after ???, fragments after --, present mode, and a deck editor in one plugin."
---
...
A deck comes from the same file the rest of your app renders as a document.
`@react-markdown-kit/slides` is one plugin with three entries: the deck, the
deck plus present mode, and the deck plus the editor commands. There is no new app to
deploy, no export step, and no second copy of the content.

It renders decks; it writes no files. PDF and PPTX are what Marp and Slidev do, and the
<Link to="/marp-alternative">comparison with Marp, Slidev and reveal.js</Link> says which
tool fits which job.
...
left, the deck on the right, Present in the control bar. Add
<a href={`${sites.slidesDemo}/?embed=1`}><code>?embed=1</code></a> to drop the page chrome,
which is the URL to put in an `iframe` in a blog post or in your own docs.

The rest of this page is the reference: the dialect, then present mode, then the editor
commands.
...
- **No script, no class, no inline style, no id other than
  `slide-<name>`.** Optional attributes appear only when set, and
  <a href={blob('plugins/slides/tests/extension.test.tsx')}>extension.test.tsx</a> asserts
  the deck's markup, attribute order included, character for character for a three-slide
  deck; the block above is slide 2 of it, copied from that expectation.
- **Server-renderable.** The deck is built as hast by a handler for the mdast
  ...
  decks on this page were rendered when the site was built, and
  <a href={blob('plugins/slides/tests/present.dom.test.tsx')}>present.dom.test.tsx</a>
  renders the present entry on the server and hydrates it without a mismatch.
...
- Every untouched block is written back from its original bytes. A deck
  loaded and saved without edits is unchanged, byte for byte
...
| Entry | Loads | Adds |
| --- | --- | --- |
| `@react-markdown-kit/slides` | nothing beyond the parser (<a href={blob('tests/packaging.test.ts')}>packaging.test.ts</a>) | the extension: dialect, diagnostics, the static deck, serialization |
...
Those three export decks to files: PDF and PPTX from a command line, or print to PDF
from the browser. This plugin does neither. It renders a deck inside a React app, from
Markdown the app already renders, with present mode and speaker notes. The
<Link to="/marp-alternative">comparison page</Link> puts the four side by side, with the
Marp directives mapped onto this dialect.

## Next

<a href={sites.slidesDemo}>The slides demo</a> ·{' '}
<a href={`${sites.slidesDemo}/?embed=1`}>The demo in embed mode</a> ·{' '}
<Link to="/marp-alternative">Marp and Slidev alternative</Link> ·{' '}
<a href="https://www.npmjs.com/package/@react-markdown-kit/slides">@react-markdown-kit/slides on npm</a> ·{' '}
<Link to="/docs/editor/basics">The editor</Link> ·{' '}
