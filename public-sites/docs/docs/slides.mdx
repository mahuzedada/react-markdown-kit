...
Markdown presentations in React, from the same file the rest of your app renders as a
document. `@react-markdown-kit/slides` is one plugin with three entries: the deck, the
...
- **No script, no class, no inline style, no id other than
  ...
  the markup above character for character.
