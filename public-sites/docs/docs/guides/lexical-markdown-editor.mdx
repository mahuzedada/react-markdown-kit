...
export const FAQ = [
  ...
  },
  {
    ...
    answer:
      'No. Nothing the root entry exports requires a Lexical type, and a test reads the published type declarations and fails if any of them imports from lexical or @lexical/*. The escape hatch, editor.getNativeEditor(), returns unknown, so you cast it yourself.',
  },
  {
...
Nothing on the root entry names Lexical. A test in the package reads the published type
declarations and fails if any of them imports from `lexical` or `@lexical/*`.
