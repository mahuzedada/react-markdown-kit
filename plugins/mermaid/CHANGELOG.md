...
- `sequenceDiagram`: participants and actors, boxes, the ten message arrows, activation, notes, `loop`, `alt`/`else`, `opt`, `par`/`and`, `critical`/`option`, `break`, `rect`, `autonumber` and titles, rendered as static SVG from a deterministic layout. Edited as text.
...
- Editor: the block header shows the kind and support level; flowcharts toggle between the canvas and text; other kinds open in a source editor with a live preview and the problems listed; one insert button per kind (`diagram`, `diagram-<kind>`); a lossy flowchart mounts read-only until acknowledged.
