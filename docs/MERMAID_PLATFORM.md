...
- `lossy` names, fixed strings: `subgraph`, `edge-style` (invisible links, long arrows, and a start marker that differs from the head, such as `<--o`; dotted, thick, circle and cross links are modelled), `shape` (a bracket written back as another shape: `[/ /]`, `[\ \]`, `{{ }}`, `((( )))`, `(( ))`, since the writer emits `[ ]`, `( )` and `([ ])`), `linkStyle`, `style-property` (a `style` declaration other than `fill`, `stroke`, `stroke-width`, `stroke-dasharray` and `color`), `markdown-string` (a backtick label whose emphasis markers the drawing cannot show). `linkStyle` lines are not retained.
...
### 9.6 The detached tool row

A canvas renders its tool row along its own top edge. The editor entry also exports `<DiagramToolbar>`, a slot a host mounts anywhere in its own chrome (inside `<MarkdownEditorProvider>` or with `editor={useMarkdownEditor(...)}`; the plugin reads the `LexicalEditor` through `getNativeEditor()`), and while one is mounted every canvas of that editor portals its row into it instead. This is the canvas counterpart of the editor package's composable route: the text surface keeps its behaviour and the host owns the chrome.

The slot is per scope, registered in `canvas/toolbar-slot.tsx` against the host's `scope` object (the `LexicalEditor` in a document, 9.7); nothing is module state. It shows one canvas's row at a time: the canvas that last had focus, or, before any has, the first one that mounted; when the owner unmounts (deleted, switched to text, changed kind) the oldest remaining canvas takes over. Canvases that do not own the slot render no row while one is mounted. A canvas under the lossy lock has no row and never registers. `useToolbarHost(scope, nodeKey, editable, active)` gives a canvas `{ detached, host }`: inline when nothing is mounted, the host element while it owns the slot, nowhere otherwise.

A detached row still belongs to its canvas: React events cross the portal, so focus in the row runs the root's `onFocus` and the root's `onBlur` treats the host as inside; the canvas keeps an `active` state from those handlers and puts `is-active` on the row, which shows the property bar where the inline `:focus-within` rule cannot. Native `keydown` listeners on the root do not hear the row, which `isControlKey` excluded anyway. The row carries `is-detached` in the slot.

The slot's root carries `rmk-editor`, so every `--rmk-diagram-*` token a host maps on that class reaches it wherever it sits, `rmk-diagram-toolbar-host`, and `data-rmk-diagram-toolbar` (`filled` or `empty`); the plugin stylesheet undoes the editor frame on it. `placeholder` renders in `.rmk-diagram-toolbar-placeholder` while no canvas owns the slot; the portal target is a child React never gives children of its own, so the placeholder and the row never contend for one node. The row keeps its inline look in the slot; a host restyles it.

### 9.7 The canvas host and the standalone canvas

A canvas component never imports Lexical or the editor package. It reads a `DiagramCanvasHost` from React context (`canvas/host.tsx`): `scope` (the object its tool row slot is keyed on), `options` (`DiagramCanvasOptions`), `labels`, `chrome` (`block` or `app`), `toolbarOrientation` (`horizontal` or `vertical`), `onFocusChange(id | null)` and `undo()`. The editor's block provides a host built from the `LexicalEditor`: the per-editor options (`node/options-store.ts`), the `diagram.*` labels, `chrome: 'block'`, focus through `DIAGRAM_FOCUS_COMMAND`, undo through `UNDO_COMMAND`. `DiagramKindEditorProps.nodeKey` is a `string`.

The `/canvas` entry (`src/canvas.ts`) exports `<MermaidCanvas>` and `createMermaidCanvas(container, options)`, the kind factories, `MERMAID_KEYWORDS`, `DIAGRAM_LABELS` and the editor contract types. `<MermaidCanvas>` is a host of its own: `chrome: 'app'` (no block-width presets, no height grip), a vertical tool row when `toolbar` is `left`, its instance as the slot scope, and a `ToolbarSlotHost` it positions as a floating island over the surface, so a `zoom` transform on the surface never scales the tools. It detects the kind, parses with the shared memoised `parseDiagramSource`, mounts the kind's canvas when the kind writes and has one, and otherwise the kind's `render` output or the source. The lossy lock and its notice behave as in the block. It keeps its own history: `commit(source, { merge })` pushes an entry, or replaces the top one when `merge` is set and the previous commit is under 300 ms old; a `value` prop that differs from the current entry is pushed as an outside change and ends the burst; Cmd/Ctrl+Z, Shift+Cmd/Ctrl+Z and Ctrl+Y move through the entries unless the key is typed into a field. `createMermaidCanvas` renders the component into the given element with `react-dom/client` and returns `{ getValue, setValue, update, destroy }`. The entry's bundle imports `react`, `react-dom/client` and `hast-util-to-jsx-runtime` and nothing else outside the package.

### 9.8 Multi-selection on the flowchart canvas

The selection follows Excalidraw (studied in its source, `packages/element/src/resizeElements.ts`, and in the app). Shift+click toggles a shape, a background drag selects the shapes fully inside the marquee, Cmd/Ctrl+A selects all. With two or more selected, `GroupSelectionOverlay` outlines each member and draws a dashed frame around the union of the members' boxes and connector paths (`selectionBounds`), with corner handles and, on a side at least 64 units long, side handles.

- A corner handle scales uniformly about the opposite corner by the larger of the two axis ratios; a side handle scales along its axis; Alt at drag start scales about the centre. Crossing the anchor clamps instead of flipping, and no box shrinks below 8 units. Positions, sizes and waypoints scale; a free text shape keeps its size and moves with its centre; stroke widths do not scale. Bound connectors follow through the binding resolver (`groupScale`, `scaleShapes` in `canvas/interaction.ts`).
- A pointer press inside the frame (8 units of slack) that is not on a member moves the whole selection. A press on a member moves the selection; released without moving, it narrows the selection to that member.
- Arrow keys move the selection by 1 unit, 10 with Shift; consecutive nudges merge into one history entry under the 300 ms rule.
- A move released within the click tolerance writes nothing, so selecting never rewrites hand-written source. A release applies its own point before committing, so a move still waiting for its animation frame is not lost.
...
- `canvas-group.test.ts`: the group scale for corner, side and Alt handles, the minimum size clamp, waypoint and text scaling, and the nudge.
- `canvas-standalone.dom.test.tsx`: `<MermaidCanvas>` floats the tools in its island without width presets, nudges and undoes and redoes through its own history, scales a two-box selection from the frame's corner, shows a kind without a canvas as source, and `createMermaidCanvas` mounts, swaps the source and unmounts.
...
- Since 0.3.0 the page is laid out like Excalidraw and runs on the standalone `<MermaidCanvas>` (9.7), not on the Markdown editor: the stage is one window tall (`100dvh`) and full width, the canvas is the whole stage, and everything else floats over it as islands: the brand and status chip top left, Docs, GitHub, Copy link, Share, the theme and the code panel toggle top right, the plugin's tool island down the left edge under the brand, the code panel (Mermaid source, samples, export) on the right, zoom and fullscreen bottom left, a link to the docs bottom centre and the version bottom right. The drawing centres in the room the islands leave (padding on the flowchart stage and the sequence viewport). The landing copy follows the stage, so the page scrolls past the editor to the documentation (`#docs`). The mermaid.live description in the next items describes the layout before 0.3.0; the preset, samples, status chip and export are unchanged.

- The preset is `mermaid({ style: 'ink', sourceEditor: false, kinds, align: 'center' })`, exported with `KINDS` from `src/preset.ts` so the demo test mounts the same block the page shows. The left code pane stays the source editor. The sequence canvas ignores `style`. `align: 'center'` is the page-as-canvas placement: the flowchart canvas centres the drawing in the room the pane gives it on both axes and scales a taller drawing down to fit it (`DiagramCanvasOptions.align`, `drawing-canvas.tsx`); the sequence canvas fits a taller picture the same way and the demo CSS centres the stage with auto margins in a flex column. Neither moves the diagram's coordinates, and a gesture never re-centres the drawing under the pointer: the flowchart canvas centres on the model it last adopted from outside.
...
- Samples are grouped, three per kind, drawn from what an enterprise team documents: Flowcharts (an order platform, a purchase approval, a data platform) and Sequence diagrams (single sign-on with alt/else, a checkout with activation, par and a note, a nightly reconciliation with loop, alt and autonumber). Every sample compiles with no diagnostic, uses no feature the canvas flattens (so it opens editable at once), has no cycle (whose back edge the auto layout would draw over the forward one), keeps to four ranks top-down or three left-to-right so it sits whole on the pane at 100% on a laptop screen, and contains no raw `;` in text.
...
- The canvas is the page, as on mermaid.live: the right pane has no frame, keeps the same page gap as the cards on every side, draws mermaid.live's dotted grid (2px dots every 30px) on the page background, and the flowchart surface and the sequence viewport are made transparent so the grid is the pane's. Pills float in its corners: the plugin's tool row top left (where mermaid.live's "Edit visually" pill sits), the view controls (fullscreen, zoom) top right, the version and the theme toggle bottom right; the header keeps brand, Docs, GitHub, Copy link and Share. The demo assembles the editor from `useMarkdownEditor`, `<MarkdownEditorProvider>` and `<MarkdownEditorContent>` so the page owns the chrome: the tool row renders into a `<DiagramToolbar>` (9.6) in the top-left pill, with a placeholder naming why there are no tools for a source-only kind; the block's header (the kind the status chip already names, and the canvas/text toggle the code pane makes redundant) and the height grip (the pane governs the height) are hidden. The drawing starts under the band the pills occupy (a top padding on the flowchart stage and the sequence viewport). Fullscreen takes the pane, pills included. `tests/mermaid-demo.dom.test.tsx` mounts the same composition and asserts the row is in the slot, not on the canvas.
