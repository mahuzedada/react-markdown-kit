...
The slot is per editor, registered in `canvas/toolbar-slot.tsx` against the `LexicalEditor` like the block options; nothing is module state. It shows one canvas's row at a time: the canvas that last had focus, or, before any has, the first one that mounted; when the owner unmounts (deleted, switched to text, changed kind) the oldest remaining canvas takes over. Canvases that do not own the slot render no row while one is mounted. A canvas under the lossy lock has no row and never registers. `useToolbarHost(editor, nodeKey, editable, active)` gives a canvas `{ detached, host }`: inline when nothing is mounted, the host element while it owns the slot, nowhere otherwise.
