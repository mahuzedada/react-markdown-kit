...
export interface RetainedLine {
  /** 1-based line inside the fence body. */
  readonly line: number
  /** The line, byte for byte. For a `%%` comment after a statement on the same line, the comment text from `%%` to the end of the line. */
...
| Message | `A->B: text` with `->`, `-->`, `->>`, `-->>`, `<<->>`, `<<-->>`, `-x`, `--x`, `-)`, `--)` | `+`/`-` after the arrow sets `activate`; actor names may contain spaces, hyphens, `(` and `)` but never `+ < > : , ;`, and the target cannot start with `(` |
...
A parse reports at most one problem per code and line; a line with several residues after `;` yields one `SEQUENCE_DIAGRAM_UNKNOWN_STATEMENT`.
...
Commits: every structural change is one history entry. For an inline edit the first change is a plain commit, so it never folds into the structural entry that opened the field (a drawn message, a wrap, a section), and the rest of the burst commits with `merge: true` under the block's 300 ms rule, so a burst of typing undoes as one step. The canvas never blocks a gesture; an operation that would make Mermaid.js reject the fence is not offered.

Read-only editors, and the lossy lock of 9.2, render the picture only: no tool row, no interaction layer. The root carries `rmk-diagram-canvas` and `rmk-sequence-canvas`, so the property-bar host and the insert focus of 9.3 follow the flowchart's rules. Labels for every control come from the `diagram.` label map so hosts can translate them.
...
- `sequence-operations.test.ts`: every operation of 9.5, its round trip through `write` and `parse`, and that no input is mutated.
